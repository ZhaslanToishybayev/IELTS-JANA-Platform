            ...
            <AnimatePresence mode="wait">
                {phase === 'LANDING' && (
                    <motion.div key="landing" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
                        <div className="space-y-2 py-4 md:py-6 mb-6 md:mb-10">
                            <h1 className="text-2xl md:text-4xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2 md:gap-3">
                                <Trophy className="w-7 h-7 md:w-10 md:h-10 text-amber-500" />
                                IELTS Mock Test
                            </h1>
                            <p className="text-sm md:text-slate-500 font-medium tracking-tight text-slate-500">Take a full practice test under real IELTS conditions.</p>
                        </div>
                            ...
                            {SECTIONS.map((section) => {
                                ...
                                return (
                                    <motion.button
                                        ...
                                        disabled={loading}
                                        className={`card p-5 md:p-8 text-left relative overflow-hidden group ${completed ? 'ring-2 ring-emerald-500' : ''}`}
                                    >
                                        <div className="flex items-start justify-between mb-3 md:mb-6">
                                            <div className={`w-10 h-10 md:w-14 md:h-14 rounded-xl md:rounded-2xl flex items-center justify-center ${section.bgLight} ${section.bgDark}`}>
                                                <Icon className={`w-5 h-5 md:w-7 md:h-7 ${section.text}`} />
                                            </div>
                                            {completed ? (
                                        ...
                                        <h3 className="text-base md:text-xl font-black text-slate-900 dark:text-white mb-1 md:mb-2">{section.title}</h3>
                                        <p className="text-xs md:text-sm text-slate-500 dark:text-slate-400 font-medium line-clamp-2">{section.description}</p>
                                        <div className="mt-3 md:mt-6 flex items-center gap-2 text-sm font-bold text-blue-600">
...
function TestScreen({ section, questions, answers, setAnswer, timeLeft, isPaused, setIsPaused, onSubmit, loading, error, listeningVoices }: {
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-4rem)] md:h-[calc(100vh-8rem)]">
            <div className="card p-3 md:p-4 mb-4 flex items-center justify-between sticky top-0 z-10 bg-white/90 dark:bg-slate-900/90 backdrop-blur">
                <div className="flex items-center gap-2 md:gap-4 min-w-0">
                    <h3 className="font-black text-slate-900 dark:text-white text-sm md:text-base truncate">{config.title}</h3>
                    <span className="text-xs md:text-sm text-slate-500 font-bold whitespace-nowrap">{answeredCount}/{questions.length}</span>
                    {isListening && (
                        <button
                            onClick={isPlaying ? stopPlayback : playAllPassages}
                            className={`hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-bold transition ${isPlaying ? 'bg-red-100 dark:bg-red-900/20 text-red-600 hover:bg-red-200' : 'bg-blue-100 dark:bg-blue-900/20 text-blue-600 hover:bg-blue-200'}`}
                        >
                            {isPlaying ? <><Square className="w-3.5 h-3.5" /> Stop</> : <><Volume2 className="w-3.5 h-3.5" /> Play All</>}
                        </button>
                    )}
                </div>
                <div className="flex items-center gap-2 md:gap-4 shrink-0">
                    {isListening && (
                        <button
                            onClick={isPlaying ? stopPlayback : playAllPassages}
                            className={`md:hidden flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs font-bold transition ${isPlaying ? 'bg-red-100 dark:bg-red-900/20 text-red-600' : 'bg-blue-100 dark:bg-blue-900/20 text-blue-600'}`}
                        >
                            {isPlaying ? <Square className="w-3.5 h-3.5" /> : <Volume2 className="w-3.5 h-3.5" />}
                        </button>
                    )}
                    <div className={`flex items-center gap-1.5 font-mono text-base md:text-lg font-black ${timeLeft < 300 ? 'text-red-500' : 'text-slate-900 dark:text-white'}`}>
...
function WritingTestScreen({ prompts, answers, setAnswer, timeLeft, isPaused, setIsPaused, onSubmit, loading, error }: {
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-4rem)] md:h-[calc(100vh-8rem)]">
            <div className="card p-3 md:p-4 mb-4 flex items-center justify-between sticky top-0 z-10 bg-white/90 dark:bg-slate-900/90 backdrop-blur">
                <div className="flex items-center gap-2 md:gap-4 min-w-0">
                    <h3 className="font-black text-slate-900 dark:text-white text-sm md:text-base">Writing</h3>
                    <div className="flex items-center gap-1 md:gap-2 text-xs md:text-sm font-bold">
                        <button onClick={() => setActiveTask('task1')} className={`px-2 md:px-3 py-1 md:py-1.5 rounded-lg transition ${activeTask === 'task1' ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>
                            Task 1
                            <span className="ml-1 text-xs font-mono hidden md:inline">{formatTime(t1Remaining)}</span>
                        </button>
                        <button onClick={() => setActiveTask('task2')} className={`px-2 md:px-3 py-1 md:py-1.5 rounded-lg transition ${activeTask === 'task2' ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>
                            Task 2
                            <span className="ml-1 text-xs font-mono hidden md:inline">{formatTime(t2Remaining)}</span>
                        </button>
                    </div>
                </div>
                <div className="flex items-center gap-2 md:gap-4 shrink-0">
...
function SpeakingTestScreen({ prompts, answers, setAnswer, timeLeft, isPaused, setIsPaused, onSubmit, loading, error }: {
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-4rem)] md:h-[calc(100vh-8rem)]">
            <div className="card p-3 md:p-4 mb-4 flex items-center justify-between sticky top-0 z-10 bg-white/90 dark:bg-slate-900/90 backdrop-blur">
                <div className="flex items-center gap-2 md:gap-4 min-w-0">
                    <h3 className="font-black text-slate-900 dark:text-white text-sm md:text-base">Speaking</h3>
                    <div className="flex items-center gap-0.5 md:gap-1 text-xs md:text-sm font-bold">
                        {parts.map(p => (
                            <button key={p} onClick={() => switchToPart(p)} className={`px-1.5 md:px-2 py-1 rounded-lg transition text-xs flex items-center gap-0.5 md:gap-1 ${activePart === p ? 'bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-300' : partProgress.find(pp => pp.part === p)?.hasContent ? 'text-emerald-500' : 'text-slate-500 hover:text-slate-700'}`}>
                                {p.replace('part', 'P')}
                ...
                <div className="flex items-center gap-2 md:gap-4 shrink-0">
                    <div className="text-center">
                        <div className={`font-mono text-base md:text-lg font-black ${phaseTimeLeft < 30 ? 'text-red-500' : 'text-slate-900 dark:text-white'}`}>
                            {formatTime(phaseTimeLeft)}
                        </div>
                        <div className="text-[9px] md:text-[10px] text-slate-400 font-bold uppercase">
                            {speakingPhase === 'prep' ? 'Prep' : 'Speaking'}
                        </div>
                    </div>
                    <button onClick={() => setIsPaused(!isPaused)} className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition">
                        {isPaused ? <Play className="w-5 h-5" /> : <Pause className="w-5 h-5" />}
